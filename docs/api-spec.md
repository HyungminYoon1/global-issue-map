...
대륙별, 키워드별 뉴스를 빠르게 조회하기 위해 아래 인덱스를 권장한다.
...
API 구현 순서는 사용자 흐름(홈 → 카테고리 → 대륙 → 검색 → 상세 → 저장)을 기준으로 한다.
...
본 API는 **홈에서 전체 이슈 탐색 → 대륙/키워드/카테고리별 검색 → 기사 상세/AI 분석 확인 → 저장 기사 관리** 흐름을 기준으로 설계되었다.
